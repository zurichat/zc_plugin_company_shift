...
const sendErrorInProduction = (errorObject, res) => {
	...
	if (
		errorObject.name === "ForbiddenError" &&
		errorObject.code === "EBADCSRFTOKEN"
	) {
	...
	if (errorObject?.error) {
		if (
			errorObject?.error.code === "ENOTFOUND" ||
			errorObject?.error.code === "ECONNRESET"
		) {
			errorObject = new AppError(
				"An error occured while connecting to a required external service",
				500
			);
		}
	}
...
module.exports = (errorObject, req, res, next) => {
	...
	process.env.NODE_ENV === "production" &&
		sendErrorInProduction(errorObject, res);
};
